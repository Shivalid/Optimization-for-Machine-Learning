...
class Grid:
	...
	def edges(self, u,isotropic=False):
		if isotropic:
			edges_=[]
			neigh_iso=self.neighbors(u, True)
			neigh_no_iso=self.neighbors(u, False)
			#print('u',u)
			for v in neigh_iso:
				
				if v in neigh_no_iso:
					#print('no_iso',v)
					edges_.append(self.edge(u, v))
				else:
					#print('iso',v)
					edges_.append(self.edge(v,u))
			return edges_
			
		else:
			return [self.edge(u, v) for v in self.neighbors(u, isotropic)]
